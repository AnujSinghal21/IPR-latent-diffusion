...
def get_dataset(dataset_name, metadata=False, synthetic_train_path=None):
    if dataset_name == 'roc':
        ...
        dataset = load_dataset('pietrolesci/agnews', 'default')
...
def process_ag_news_dataset(dataset):
    def process_ag_news_text(example):
        ...
        return {'text': PreTrainedTokenizerBase.clean_up_tokenization(example["text"].strip()), 'labels':example['labels']-1}
    dataset = dataset.map(process_ag_news_text, remove_columns=['labels', 'uid', 'text'])
    return dataset
